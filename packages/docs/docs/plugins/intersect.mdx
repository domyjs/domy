...
import intersect from '@domyjs/intersect';

# Intersect

The `d-intersect` directive allows you to **trigger an action when an element appears in the viewport**, making it perfect for **lazy loading, animations, or analytics tracking**.
...
```js
DOMY.createApp().plugins([intersect]).mount();
```
...
```html
<img d-intersect="loadImage($el)" data-src="image.jpg" alt="Lazy-loaded image" />
```
