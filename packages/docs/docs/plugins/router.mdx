...
router.beforeEach(({ to, from }) => {
  // Return types:
  // - undefined/void: continue navigation
...
Access router functionality in components:

```typescript
const router = useRouter(); // or router.getHelper()
```
...
#### `navigate(routeInfos: RouteInfos)`

Programmatically navigate:

```typescript
router.navigate({
  name: 'user-profile', // or path: '/user-profile' instead of name
  params: { id: 123 },
  queryParams: { tab: 'settings' }
});
```

#### `replace(routeInfos: RouteInfos)`
